...
> 도마뱅 생체로봇 기반고난도 재난 접근 로봇
> 

```markdown
https://youtu.be/2gfw7_b34kY
```
...
### 5-1. 음성 분리·인식 및 음원 방향 추

**소스코드 설명**

4채널 마이크 입력을 이용해 재난 현장의 잡음 속에서 사람의 음성을 인식하고, 각 마이크에 소리가 도달하는 시간차를 바탕으로 음원 방향을 추정하는 기능입니다
...
def recognize_rescue_voice(audio_frame, recognizer):
    stt_audio = audio_frame[:, 0]
    audio_int16 = (stt_audio * 32767).astype(np.int16)
...
direction_vector, azimuth = calculate_direction_vector(
    mic_1, mic_2, mic_3, mic_4
)

motion = decide_motion(azimuth)

print(f"구조자 추정 방향: {azimuth:.1f} degrees")
print(f"로봇 이동 명령: {motion}")
```
```
...
역운동학(IK)을 바탕으로 다리 관절의 목표 궤적을 생성하고, 보행 시 발의 접지·이탈 시점에 맞춰 진공 흡착 및 해제 타이밍을 동기화하는 구조를 사용합니다.
